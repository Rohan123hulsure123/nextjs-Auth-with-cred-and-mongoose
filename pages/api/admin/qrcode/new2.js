        ...
        try {
          ...
          for (let i = 1; i <= qrCodeCount; i++) {
            ...
            const qrCode = new QRCodeCanvas({
              width: 200,
              height: 200,
              data: `${process.env.NEXTAUTH_URL}/view/${uid16Digit}`,
            ...
            const buff = await qrCode.toBuffer("svg")
            // console.log(buff.toString());
            const qrCodeImageData = await buff.toString();
            // console.log(qrCodeImageData);
            const modifiedSvg = qrCodeImageData.replace("</svg>", `<text x=\"60\" y=\"215\" fill=\"#0C134F\">${activationCode_6Digit.split("").join(" ")}</text></svg>`);
            const sizeModification = modifiedSvg.replace(`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="200" height="200">`, `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="200" height="220">`)
            // console.log(qrCodeImageData);
            ...
            archive.append(sizeModification, {
          ...
          archive.on("end", async () => {
            ...
            const downloadUrl = await bucket.file(zipFileName).getSignedUrl({
              action: "read",
              expires: `${date.getMonth()}-${date.getDate()}-${date.getFullYear()+1}`,
            });
